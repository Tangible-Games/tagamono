...
namespace std {
template <>
struct formatter<::Symphony::Math::Vector3d> {
  constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

  auto format(const ::Symphony::Math::Vector3d& v, format_context& ctx) const {
    return format_to(ctx.out(), "Vector3d(x:{}, y:{})", v.x, v.y, v.z);
  }
};
}  // namespace std
