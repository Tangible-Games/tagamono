...
class TransformationMatrix3d {
 public:
  ...
  void MakeTranslation(float translate_x, float translate_y,
                       float translate_z) {
    MakeIdentity();
    m41 = translate_x;
    m42 = translate_y;
  ...
  // TODO(truvorskameikin): Implement MakeRotation. Need to implememnt
  // Quaternion and Rotation classes.

  void MakePerspective(float horizontal_fov_deg, float aspect_ratio,
  ...
  TransformationMatrix3d Multiply(const TransformationMatrix3d& rhv) const {
    ...
    for (int i = 0; i < 4; ++i) {
      for (int j = 0; j < 4; ++j) {
        result_m[i * 4 + j] =
            m[0 * 4 + j] * rhv_m[i * 4 + 0] + m[1 * 4 + j] * rhv_m[i * 4 + 1] +
            m[2 * 4 + j] * rhv_m[i * 4 + 2] + m[3 * 4 + j] * rhv_m[i * 4 + 3];
      }
    }
